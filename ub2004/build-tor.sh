...
_build_tor() {
    ...
    sleep 2
    tar -Jcvf /tmp/tor-"${_tor_ver}"-1_ub2004_amd64.tar.xz *
    echo
    sleep 2
    cd /tmp
    openssl dgst -r -sha256 tor-"${_tor_ver}"-1_ub2004_amd64.tar.xz | sed 's|\*| |g' > tor-"${_tor_ver}"-1_ub2004_amd64.tar.xz.sha256
    cd /tmp
    rm -fr "${_tmp_dir}"
    rm -fr /tmp/tor
    /sbin/ldconfig
